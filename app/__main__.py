...
def synthetic_dataset() -> dict[str, tf.Tensor]:
    ...
    y_train: tf.Tensor = keras.random.randint(shape=[num_samples_train], minval=0, maxval=num_classes)

    # random validation data
    x_val: tf.Tensor = keras.random.normal(shape=[num_samples_val, num_features])
    y_val: tf.Tensor = keras.random.randint(shape=[num_samples_val], minval=0, maxval=num_classes)
...
def main(argv: list[str]) -> None:
    ...
    trainer.train(my_linear, dataset['x_train'], dataset['y_train'], dataset['x_val'], dataset['y_val'])
