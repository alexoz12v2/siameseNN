...
# choose tensorflow as keras backend (il tipo di tensore usato dipende dal backend)
...
class Linear(keras.layers.Layer):
    def __init__(self, *, units: int = 32, input_dim: int = 32) -> None:
        ...
        logging.info(f"the trainable layers are {self.trainable_weights}")
...
def main(argv: list[str]) -> None:
    ...
    out = my_linear(input)
    logging.info(f"output from MLP: {out}" f"layer.losses:    {my_linear.losses}")
